...
while :; do
  api_state="$(docker compose ps --format json api 2>/dev/null | python3 -c 'import json,sys
raw = sys.stdin.read().strip()
print(json.loads(raw).get("Health") or "unknown" if raw.startswith("{") else "unknown")' 2>/dev/null || echo unknown)"
  web_state="$(docker compose ps --format json web 2>/dev/null | python3 -c 'import json,sys
raw = sys.stdin.read().strip()
print(json.loads(raw).get("Health") or "unknown" if raw.startswith("{") else "unknown")' 2>/dev/null || echo unknown)"
...
printf '\n%s[4/4] exercising the product through %s%s\n' "${BOLD}" "${BASE}" "${RESET}"
